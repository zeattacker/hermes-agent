...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // ZEXUS: button-click reply (native Baileys buttonsResponseMessage)
      // surfaces as a normal incoming text message whose body is the
      // selectedButtonId — so downstream gateway can detect e.g.
      // "pf_confirm:5" and intercept without an LLM round-trip.
...
// ZEXUS: send a button message (interactive Ya / Batal).
